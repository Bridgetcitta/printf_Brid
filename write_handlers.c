...
int write_num(int ind, char buffer[],
	int flags, int width, int precspec,
	int length, char padd, char extra_c)
{
	int i, padd_start = 1;

	if (precspec == 0 && ind == BUFF_SIZE - 2 && buffer[ind] == '0' && width == 0)
		return (0); /* printf(".0d", 0)  no char is printed */
	if (precspec == 0 && ind == BUFF_SIZE - 2 && buffer[ind] == '0')
		buffer[ind] = padd = ' '; /* width is displayed with padding ' ' */
	if (precspec > 0 && precspec < length)
		padd = ' ';
	while (precspec > length)
		buffer[--ind] = '0', length++;
	if (extra_c != 0)
		length++;
	if (width > length)
